...
<h1>How to connect to the ProLearn SPI Target?</h1>

<%
String spiUrl = oai.getSpiTargetUrl();
if(spiUrl.equals("")){
	spiUrl = PropertiesManager.getInstance().getProperty("harvestToSpi.URI");
}
String sessionManagementUrl = oai.getSessionManagementUrl();
if(sessionManagementUrl.equals("")){
	sessionManagementUrl = PropertiesManager.getInstance().getProperty("harvestToSpi.Session.URI");
}
String username = oai.getUsername();
if(username.equals("")){
	username = PropertiesManager.getInstance().getProperty("harvestToSpi.Session.username");
}
String password = oai.getPassword();
if(password.equals("")){
	password = PropertiesManager.getInstance().getProperty("harvestToSpi.Session.password");
}
%>
			...
			<div class="install-form">
  	   			<div class="form-block">
  	     			<table class="content2">
  		  			<tr>
  		    			<td>SPI Target Service Location :<br/><input class="inputbox" style="width:100%;" type="text" class="inputboxadd" name="spiTargetUrl" value="<%=spiUrl%>"/></td>
  		    		</tr>
  		  			<tr>	 	
			  		...
			  		<tr>
  		  			...
  		  			<tr>
  		    			<td>Session Management Service Location :<br/><input class="inputbox" style="width:100%;" type="text" class="inputboxadd" name="sessionManagementUrl" value="<%=sessionManagementUrl%>"/></td>
  		    		</tr>
  		  			<tr>	 	
  		  			...
  		  			<tr>
  		    			<td>Username :<br/><input class="inputbox" type="text" class="inputboxadd" name="username" value="<%=username%>"/></td>
  		    		</tr>
  		  			<tr>	 	
			  		    <td><br></td>
			  		</tr>
  		  			<tr>
  		    			<td>Password :<br/><input class="inputbox" type="text" class="inputboxadd" name="password" value="<%=password%>"/></td>
  		    		</tr>
		  	     	</table>
  				</div>
			</div>
...
<input type="hidden" id="previous" name="previous" value="harvestToSpi"/>
