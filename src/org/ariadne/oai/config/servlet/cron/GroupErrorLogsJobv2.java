...
public class GroupErrorLogsJobv2 implements StatefulJob {
...
//		strReplaceTest();
	...
	private static void strReplaceTest() {
		String test = "Error at Line 90: cvc-datatype-valid.1.2.3: 'provider' is not a valid value of union type 'roleMeta'.";
			test = test.replaceFirst("Error at Line (.+?): ", "");
			System.out.println(test);

	}

	private static void tests() {
		GroupErrorLogsJobv2 job = new GroupErrorLogsJobv2();
	...
	protected HashMap<String, HashMap<String, TreeSet<String>>> collectErrors(File directory, String fileName) {
		...
		try {
			...
			String outputpath = directory + File.separator + "output" + File.separator;
			String recordOutputPath = outputpath + File.separator + "record" + File.separator;
			String errorOutputPath = outputpath + File.separator + "error" + File.separator;
			String errorListOutputPath = outputpath + File.separator + "errors.txt";
			File output = new File(recordOutputPath);
			if (!output.exists()) {
				output.mkdirs();
			}
			output = new File(errorOutputPath);
			if (!output.exists()) {
				output.mkdirs();
			}
			output = new File(errorListOutputPath);
			if (!output.exists()) {
				output.createNewFile();
			}
			StringBuffer fileoutputString;
			...
			Pattern recordErrorPattern = Pattern.compile("Invalid metadata found. OAI Identifier was (.+?). Error is : \\((.+?)\\)(.*)",
					Pattern.CASE_INSENSITIVE);
			
			try {
				while (str != null) {
					fileoutputString = new StringBuffer();
					String id = "";

					Matcher matcher = recordErrorPattern.matcher(str);
					while (matcher.find()) {
						id = matcher.group(1);
...
//							}
							...
							for (String singleError : collectedErrors.get(errorType).keySet()) {
								String replaced = singleError.replaceFirst("Error at Line (.+?): ", "");
								String errormd5 = IOUtilsv2.md5Hash(replaced);
								String errorMD5outputFileName = errorOutputPath + errormd5 +".txt";
								File f = new File(errorMD5outputFileName);
								if(!f.exists()) {
									String stringAppend = errormd5 + " >>>>> " + replaced;
									IOUtilsv2.appendStringToFileInEncodingUTF8(stringAppend + "\n", errorListOutputPath );

									f.createNewFile();
								}
								IOUtilsv2.appendStringToFileInEncodingUTF8(id + "\n", errorMD5outputFileName );
								fileoutputString.append(singleError + "\n");
...
//					IOUtilsv2.writeStringToFileInEncodingUTF8(fileoutputString.toString(), recordOutputPath + id.replaceAll(":", "_").replaceAll("/", ".s.") +".txt" );
					str = reader.readLine();
				}
			} catch (Exception e) {
				e.printStackTrace();
