...
import org.ariadne.spi.SpiDevTarget;

public class SpiDevWriter extends GenericWriter {

	public void CreateTarget(String spiTargetUrl) {
		SpiDevTarget target = null;
		try {
			target = new SpiDevTarget(spiTargetUrl);
		} catch (AxisFault e) {
			e.printStackTrace();
		}
		this.target = target;
	...
	public void pushAway(Vector<Record> lomVector, String repositoryIdentifier, String set) {
		((SpiDevTarget) target).submitMetadata(lomVector, repositoryIdentifier, set);
	}

	public void connect() {
		if (target != null) {
			((SpiDevTarget) target).openNewSession();
		}
	}

	public void disconnect() {
		if (target != null) {
			((SpiDevTarget) target).closeSession();
		}
	}
