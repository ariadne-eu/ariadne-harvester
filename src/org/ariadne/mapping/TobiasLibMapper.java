...
public class TobiasLibMapper extends GenericMapper {
	...
	public static void listRecords() throws OAIException, IOException, JDOMException {
		...
		repos.setBaseURL("http://tobias-lib.uni-tuebingen.de/oai2/oai2.php");
		OAIRecordList records = repos.listRecords("oai_dc","9999-12-31","2000-12-31","pub-type%3A30");
		int counter = 0;
//		records.moveNext();
		while (records.moreItems()) {
			...
			if(item != null && !item.deleted()) {
				...
				if(record.getMetadata() != null) {
					...
					IOUtilsv2.writeStringToFileInEncodingUTF8(JDomUtils.parseXml2string(mapDCToLom(record).getDocument()), "loms/" + item.getIdentifier().replaceAll(":", "_").replaceAll("/", ".s.") +".xml" );	
					IOUtilsv2.writeStringToFileInEncodingUTF8(JDomUtils.parseXml2string(record.getMetadata().getDocument()), "dcs/" + item.getIdentifier().replaceAll(":", "_").replaceAll("/", ".s.") +".xml" );
				}
				else {
...
//	public static void mapFromFile(String metadata) {
//		
//	}
	...
	public static Element mapDCToLom(Record record) throws OAIException, JDOMException {
		...
		if(titleNodes != null && titleNodes.size() > 0) {
			String titleString = titleNodes.get(0).getTextTrim();
			title = OaiUtils.newElement("title", general);
			OaiUtils.addLangString(language,titleString, title);
			if(titleNodes.size() > 1) {
				titleString = titleNodes.get(1).getTextTrim();
//				title = OaiUtils.newElement("title", general);
				OaiUtils.addLangString("en",titleString, title);
			}
		}

		//set the General Description
		List<Element> descrNodes = descriptions.selectNodes(metadata);
		if(descrNodes != null && descrNodes.size() > 0) {
			String descrString = descrNodes.get(0).getTextTrim();
			Element descr = OaiUtils.newElement("description", general);
			OaiUtils.addLangString(language,descrString, descr);
			if(descrNodes.size() > 1) {
				descrString = descrNodes.get(1).getTextTrim();
				
				if(title == null) {
					title = OaiUtils.newElement("title", general);
				}
				OaiUtils.addLangString("en",descrString, title);
			}
		}
		...
		for (Element identifierEl : identifierNodes) {
			...
			if(id.startsWith("http")) {
				...
				identifier = new Element("identifier", JDomUtils.LOMNS);
				general.addContent(0, identifier);
				JDomUtils.addIdentifier("URN", id, identifier);
			}
		}
