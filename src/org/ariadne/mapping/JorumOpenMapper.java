...
public class JorumOpenMapper extends GenericMapper {
	...
	protected static XPath rights = createOaiDcXpath("dc:rights");
	...
	public static void listRecords() throws OAIException, IOException, JDOMException {
		...
		repos.setBaseURL("http://open.jorum.ac.uk/oai/request");
...
//					System.out.println(datestr);
					...
					try {
						//Tue, 22 Sep 2009 15:21:35 +0100
						fmt2 = new DateTimeFormatterBuilder().appendDayOfWeekShortText().appendLiteral(", ")
		...
		List<Element> formatNode = formats.selectNodes(metadata);
		for (Element formatsEl : formatNode) {
			String formatsString = formatsEl.getText(); 
		...
		if(rightsNodes != null) {
			...
			if(url) {
				...
				OaiUtils.addLangString(language, descr, rightsDescr);
			}
		}
